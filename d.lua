...
function components.Dropdown(holder, options, zindex)
    ...
    if not options.ignored and library.config_objects[options.flag] then
        return library.config_objects[options.flag]
    end
    ...
    local search_input = dropdown:Create("Input", {
        Text = "",
        Placeholder = "Search...",
        Font = library.font,
        Size = library.font_size,
    ...
    local function refresh_list(filter)
        for _, opt in next, option_objects do
            opt.object:Destroy()
        end
        table.clear(option_objects)
        for _, name in next, active_content do
